			...
			if ( ! empty( $final_data ) ) {
				try {
					...
					if ( empty( $access_token ) ) {
						error_log( 'SAIFGS CF7: No access token available for Google Sheets insertion.' );
					...
					if ( is_wp_error( $response ) ) {
						error_log( 'SAIFGS CF7: Google Sheets API request failed - ' . $response->get_error_message() );
					...
					if ( $response_code === 401 ) {
						error_log( 'SAIFGS CF7: Authentication failed (401) when inserting data to Google Sheets.' );
					...
					if ( $response_code !== 200 ) {
						error_log( 'SAIFGS CF7: Google Sheets API returned error code: ' . $response_code . ' - Body: ' . $body );
					...
					error_log( 'SAIFGS CF7: Successfully inserted data to Google Sheet. Row ID: ' . $sheet_tab_row_id );

				} catch ( \Exception $e ) {
					error_log( 'SAIFGS CF7: Exception in Google Sheets insertion - ' . $e->getMessage() );
